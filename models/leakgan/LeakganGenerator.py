...
class Generator(object):
    def __init__(self, sequence_length, num_classes, num_vocabulary,
        ...
        with tf.variable_scope('Worker'):
            ...
            self.image_feat_W = tf.get_variable("image_feat_W", [self.image_feat_dim, self.emb_dim], tf.float32, random_uniform_init)
            self.image_feat_b = tf.get_variable("image_feat_b", [self.emb_dim], tf.float32, random_uniform_init)
        ...
        image_feat_emb = tf.matmul(image_feat, self.image_feat_W) + self.image_feat_b 

        with tf.variable_scope("lstm_encoder") as scope:
            _, state = self.encoder(image_feat_emb, state)
            lstm_c, lstm_h = state
            self.h0_worker = tf.stack([lstm_h, lstm_c])  #where h0_worker is the initial state for decoder
    ...
    def create_Worker_recurrent_unit(self, params):
        with tf.variable_scope('Worker'):
            ...
            def unit(x, hidden_memory_tm1):
                #print("hidden_memory_tm1:"+str(tf.shape(hidden_memory_tm1)))
                previous_hidden_state, c_prev = tf.unstack(hidden_memory_tm1)
                #print(previous_hidden_state)
                #print(c_prev)
